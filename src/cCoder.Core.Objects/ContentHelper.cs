...
{
    private const string tag = @"\[TYPE\[[A-Za-z\d_/-]*\][A-Za-z\d_/-]*\=*\""*-*[A-Za-z\d_/-]*\""*\]";
    ...
    public static string ProcessContentString(string key, RenderParams p, string content, IEnumerable<Replacement> replacements)
    {
        ...
        TimeSpan validateStartTime = DateTimeOffset.Now - renderStart;
    ...
    private static void Nav(StringBuilder result, PageRenderParams prp)
    {
        ...
        {
            return string.Join("", prp.App.Pages
                    ...
                    .Select(s =>
                    {
                        ...
                        string selected = s.ParentId != null && page != null && prp.Page.Path.Contains(s.Path) ? " active" : string.Empty;
                        return expand
                            ? $"<li data-id='{s.Id}' class='nav-item'><a href='/{s.Path}' class='nav-link{selected}'>{s.Title(prp.Culture)}</a><ul class='submenu dropdown-menu'>{BuildMenuItemsFor(s, expand)}</ul></li>"
                            : $"<li data-id='{s.Id}' class='nav-item'><a href='/{s.Path}' class='nav-link{selected}'>{s.Title(prp.Culture)}</a></li>";
                    }));
        }
        ...
        result.RegexReplace(tag.Replace("TYPE", "nav"), (m) =>
        {
            ...
            Page page = prp.App.Pages.FirstOrDefault(p => p.Id == pageId);
    ...
    private static (string, string, string[]) SplitMatch(System.Text.RegularExpressions.Match match)
    {
        string[] tagParts = match.ToString().Split("[", StringSplitOptions.None);
        string[] tagParts2 = tagParts.Last().Split("]", StringSplitOptions.None);
    ...
    private static string Content(string key, Content content, PageRenderParams p, (string type, string name, string[] options) tag, IEnumerable<Replacement> replacements)
    {
        ...
        string optionalClass = string.Join(" ", tag.options
    ...
    private static void Resource(string key, StringBuilder source, RenderParams p, IEnumerable<Replacement> replacements)
    {
        ...
        foreach (string resourceName in namesInKey)
        {
    ...
    {
        Resource existsForCurrentCulture = ObjectCache.Get<Resource>($"resource|{key}-{name}-{culture}");
        ...
        if (culture.Contains('-'))
        {
            ...
            Resource existsForPrimitiveCulture = ObjectCache.Get<Resource>($"resource|{key}-{name}-{primitiveCulture}");
    ...
    private static void Meta(StringBuilder source, string culture) =>
    ...
    private static IEnumerable<Replacement> BuildThemeReplacements<T>(T model, string prefix = "")
    {
    ...
    private static IEnumerable<Replacement> BuildIEnumerableThemeReplacements<T>(T model, string prefix) => model.GetType()
            .GetProperties()
            .SelectMany(p =>
            {
