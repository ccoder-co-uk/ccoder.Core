...
namespace cCoder.Core.Data
{
    ...
    {
        static readonly string[] sysMethods = new[] { "ToString", "Equals", "GetHashCode", "GetType" };

        IEnumerable<int> AdminOf => User.Roles?.Where(r => r.Role.Privileges.Any(p => p == "app_admin")).Select(r => r.Role.AppId) ?? Array.Empty<int>();

        IEnumerable<Guid> CurrentUserRoleIds => User.Roles?.Select(r => r.RoleId) ?? Array.Empty<Guid>();
        ...
        static void ConfigureModel(ModelBuilder builder)
        {
