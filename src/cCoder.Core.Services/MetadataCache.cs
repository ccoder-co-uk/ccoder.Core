...
public class MetadataCache : Cache<MetadataContainerSet>, IMetadataCache
{
    ...
    public string GetAll(string culture = "") => 
        "[" + string.Join(',', typeSet.Select(c => metaSerialized[culture][c.Name.ToLower()]).ToArray()) + "]";
    ...
    public string ToJson(string culture) => 
        metaSerialized[culture].ToJsonForOdata();

    string IMetadataCache.Get(string key, string culture) => 
        metaSerialized[culture].ContainsKey(key) 
            ? metaSerialized[culture][key] 
            : string.Empty;
}