...
namespace Core.Objects.Entities.CMS
{
    ...
    public class Page : IAmRoleSecured<PageRole>
    {
        ...
        [DontPrivilege]
        public string Title(string culture) => InfoForCulture(culture).Title ?? string.Empty;

        [DontPrivilege]
        public string Description(string culture) => InfoForCulture(culture).Description ?? string.Empty;

        [DontPrivilege]
        public string Keywords(string culture) => InfoForCulture(culture).Keywords ?? string.Empty;
        ...
        public PageInfo InfoForCulture(string culture)
        {
            ...
            return orderedInfo.FirstOrDefault(i => culture == i.CultureId || culture.Contains(i.CultureId)) ?? orderedInfo.FirstOrDefault();
        }
        ...
        public async Task UpdateContents(User user, ICollection<Content> newContents, ICoreDataContext core)
        {
            ...
            // apply updates
            Contents.ForEach(c => c.Html = newContents.FirstOrDefault(nc => c.Name == nc.Name && c.CultureId == nc.CultureId)?.Html ?? c.Html);

            // add the new stuff
            List<Content> addedContents = newContents.Where(nc => !Contents.Any(c => c.Name == nc.Name && c.CultureId == nc.CultureId)).ToList();
            ...
            IEnumerable<Content> removedContents = Contents.Where(c => c.CultureId != string.Empty && !newContents.Any(nc => c.Name == nc.Name && c.CultureId == nc.CultureId));
        ...
        public async Task UpdateInfo(User user, ICollection<PageInfo> newInfo, ICoreDataContext core)
        {
            ...
            // add the new stuff
            PageInfo[] addedInfo = newInfo?.Where(ni => ni.CultureId != string.Empty && !PageInfo.Any(pi => pi.CultureId == ni.CultureId)).ToArray() ?? Array.Empty<PageInfo>();
            addedInfo.ForEach(c =>
            {
                c.PageId = Id;
                c.Id = 0;
            ...
            // remove the old stuff
            List<PageInfo> removedPageInfos = PageInfo.Where(c => c.CultureId != string.Empty && !newInfo.Any(nc => c.CultureId == nc.CultureId)).ToList();
