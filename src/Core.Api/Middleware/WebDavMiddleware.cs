...
namespace Web.Api.Middleware
{
    public class WebDavMiddleware
    {
        ...
        public async Task InvokeAsync(HttpContext context, ICoreDataContext ctx, Config config)
        {
            ...
            try
            {
                switch (context.Request.Method.ToUpper())
                {
                    ...
                    case "GET":
                        int getVer = query.ContainsKey("version") ? int.Parse(query["version"][0]) : 0;
                    ...
                    case "PROPPATCH":
                        string responseXmlElement = new XElement(ns + "multistatus", new object[] {
                    ...
                    case "PUT":
                        ...
                        await Respond(context, context.Request.Body, context.Request.Headers["Content-Type"], headers);
                        break;
                    ...
                    case "DELETE":
                        await dms.Drop(path, query.ContainsKey("version") ? int.Parse(query["version"][0]) : 0);
                        await Respond(context, null, headers);
                        break;
        ...
        {
            XDocument requestBody = requestText.Length > 0 && context.Request.Headers["Content-Type"] == "application/xml" 
        ...
        string PropFindFile(ICoreDataContext ctx, int appId, Path path, XNamespace ns, string urlBase, IEnumerable<string> requestedProperties)
        {
            try
            {
                File file = ctx.GetAll<File>(false)
                    ...
                    .Include(k => k.Folder)
                    .FirstOrDefault(f => f.Folder.AppId == appId && path.Length > 0 && f.Path == path.FullPath.ToLower());
        ...
        {
            Folder folder = path.FullPath != "" ? ctx.GetAll<Folder>(false)
            ...
            {
                Name = "Root",
                SubFolders = ctx.GetAll<Folder>(false).Where(f => f.AppId == appId && f.ParentId == null).ToArray(),
                Files = new List<File>(),
                Path = "",
                AppId = appId
            };
            ...
            {
                folders = ctx.GetAll<Folder>(false).Where(f => f.AppId == appId && (path.FullPath != "" ? f.Parent.Path == path.FullPath.ToLower() : f.ParentId == null))
                    .Include(k => k.Files)
            ...
            IEnumerable<XElement> response = folders.Select(k => k.ToWebDavResponse(urlBase, ns, requestedProperties)).Union(files.Select(k => k.ToWebDavResponse(urlBase, ns, requestedProperties)));
            string responsePropXml = new XElement(ns + "multistatus", new object[] { new XAttribute(XNamespace.Xmlns + "D", "DAV:"), new XAttribute(XNamespace.Xmlns + "Z", "urn:schemas-microsoft-com:") }
                .Union(response))
                .ToXml();
