...
namespace cCoder.Core.Api
{
    public static class IEdmModelExtensions
    {
        ...
        static MetadataContainer BuildMetaFor(IEdmType definition)
        {
        ...
        static IEnumerable<OperationContainer> GetBaseCRUDOperations(MetadataContainer type)
            => type.IsJoinEntity ? GetBaseCRUDOperationsForJoinEntity(type) : GetBaseCRUDOperationsForEntity(type);

        static IEnumerable<OperationContainer> GetBaseCRUDOperationsForJoinEntity(MetadataContainer type)
            => new List<OperationContainer>
            {
                ...
                new OperationContainer { Name = "Get All", Url = $"{type.Category}/{type.Name}", Queryable = true, HttpVerb = "GET", ReturnType = type },
                new OperationContainer { Name = "Delete", Url = $"{type.Category}/{type.Name}({{Left=leftKey,Right=rightKey}})", HttpVerb = "DELETE" },
            };

        static IEnumerable<OperationContainer> GetBaseCRUDOperationsForEntity(MetadataContainer type)
        {
            try
            {
                return new List<OperationContainer>
                {
                    ...
                    new OperationContainer { Name = "Get All", Url = $"{type.Category}/{type.Name}", Queryable = true, HttpVerb = "GET", ReturnType = type },
                    new OperationContainer { Name = "Delete", Url = $"{type.Category}/{type.Name}({{key}})", HttpVerb = "DELETE" },
                };
            }
