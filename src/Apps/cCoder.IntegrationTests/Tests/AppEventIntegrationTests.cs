...
public sealed partial class AppEventIntegrationTests(IntegrationAcceptanceFixture fixture)
{
    ...
    private async Task GrantGuestAdminAsync(int appId)
    {
        ...
        Role role = await core.Set<Role>()
            .IgnoreQueryFilters()
            .SingleOrDefaultAsync(predicate: found => found.AppId == appId && found.Name == templateRole.Name) ?? await core.AddRoleAsync(role: new Role
            {
                Id = Guid.NewGuid(),
                AppId = appId,
...
roles: [.. core.Set<Role>()
    ...
    private async Task SendWithOptionalHostAsync(HttpMethod method, string relativeUrl, string host = null)
    {
        ...
        response.StatusCode.Should()
            .Be(expected: HttpStatusCode.Accepted,because: BuildFailureMessage(content: content));
    }
