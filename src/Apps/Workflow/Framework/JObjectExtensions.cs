...
public static class JObjectExtensions
{
    ...
    private static TResult[] JArray<T, TResult>(this JArray source, Func<T, TResult> select) => source.Cast<T>().Select(select).ToArray();

    private static TResult[] JObject<T, TResult>(this JObject source, Func<T, TResult> select) => new[] { source }.Cast<T>().Select(select).ToArray();
}