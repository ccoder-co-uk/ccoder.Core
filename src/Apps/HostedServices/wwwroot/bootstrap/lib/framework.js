var draw = {
    ...
    circle: function(ctx, x, y, r, col, sel) {
        ...
        var hover = ctx.isPointInPath(mouseposition.x, mouseposition.y);
        col = hover ? (sel || col || '#cbcbcb') : (col || '#cbcbcb');
        r = hover ? (r + 1) : r;
        ctx.arc(x, y, r, 0, 2 * Math.PI);
        ctx.fillStyle = col;
        ctx.fill();
    ...
    enableShadows: function(ctx, x, y, w, h, col, offsetY) {
        ...
        ctx.shadowBlur = 6;
        ctx.shadowOffsetX = 4;
        ctx.shadowOffsetY = offsetY ? offsetY : 4;
...
class Collidable extends Drawable {
    constructor(text, x, y, w, h, r) { super(text, x, y, w, h, r); }
    collides() {
        var withinX = (mouseposition.x < this.x + (this.r || this.w)) && (mouseposition.x > this.x - (this.r || 0));
        var withinY = (mouseposition.y < this.y + (this.r || this.h)) && (mouseposition.y > this.y - (this.r || 0));
        var collision = (withinX && withinY) ? this : null;
        if (collision) { collision = this.objects ? this.objects.map(o => o.collides()).filter(r => r)[0] || this : this; }
        return collision;
    }
}
class Widget
    ...
    commandColumn() {
        ...
        this.commands.forEach(function (command) {
            if (command.template) {
                ...
                if (command.href) {
                    result += "<a class='btn btn-primary' name='" + command.name + "' href='" + command.href + "'><span class='k-icon " + command.icon + "'></span>" + command.text + "</a>";
                } else {
...
class Api
{
    ...
    onFileReaderLoad(to, file, reader, resolve, callback) {
        ...
        xhr.onreadystatechange = () => this.onReadyStateChange(xhr, file, resolve, callback);
    ...
    onReadyStateChange(xhr, file, resolve, callback) {
        switch (xhr.readyState) {
            case 2: //HEADERS_RECEIVED
                if (notification) { notification.info("Uploading file " + file.name + " ..."); }
                break;
            case 4: //DONE
                if (notification) {
    ...
    async login(user, pass, keepToken) {
        return this.send("POST", "Account/Login", { User: user, Pass: pass }).then((token) => {
            if (keepToken) {
                this.token = token.id;
            }
            return token;
    ...
    /*
     * Registration post looks like this ...
     * 
     {
    ...
    */
    ...
    addToMetaCache(metaSet) {
        for (var i in metaSet) {
            var ctx = metaSet[i];
            var existing = this.cache.meta.filter(x => x.Name == ctx.Name);
            if (existing.length > 0) {
                for (var t in ctx.Types) {
                    existing[0].Types.push(ctx.Types[t]);
                }
            }
            else { this.cache.meta.push(ctx); }
        }
    }

    addToResourceCache(resourceSet) {
        for (var r in resourceSet) {
    ...
    getResource(key, name, culture) {
        culture = culture || "";
        if (!key || !name) { return { Key: key, Name: name, DisplayName: name, ShortDisplayName: name, Description: name }; }
        ...
        return (resultSet.length > 0) ? resultSet[0] : { Key: key, Name: name, DisplayName: name, ShortDisplayName: name, Description: name };
    }
    ...
    async get(query) { return this.send("GET", query, null); }
    async add(query, model) { return this.send("POST", query, model); }
    async update(query, model) { return this.send("PUT", query, model); }
    async post(query, model) { return this.send("POST", query, model); }
    async put(query, model) { return this.send("PUT", query, model); }
    async destroy(query) { return this.send("DELETE", query, null); }

    success() { notification.success("Request Complete!"); }
}

window.api = new Api({ baseUrl: session.apiRoot, token: session.token });
var html = {
...
class Link extends Collidable {
    ...
    draw(ctx) {
        var source = this.flow.Activities.filter(a => a.model.Ref === this.model.Source)[0];
        var destination = this.flow.Activities.filter(a => a.model.Ref === this.model.Destination)[0];
    ...
    mouseup(e) {
        ...
        var container = $("[name=content]", $(d.element));
...
class Action extends Collidable {
    ...
    mouseup(e) { this.click(e); }

    draw(ctx) { draw.text(ctx, this.x + 10, this.y + 15, this.text, this.col); }
}
class Activity extends Collidable {
    constructor(meta, flow, model) {
        ...
        if (this.text.length > 20) { this.text = this.text.substring(0, 18) + "..."; }
    ...
    confirmTypes(e) {
        ...
        function next(type) {
            ...
            if (types.length === e.data.count) {
                if (e.data.callback) { e.data.callback(types.join(',')); }
            }
            if (index < e.data.count) { getType(); }
        }

        function getType() {
            var type = $('[name=type' + index + ']').data('kendoComboBox').dataItem();
            var asArray = $('[name=isArray' + index + ']:checked').length;
            if (type) { index++; next(asArrayType(type, asArray)); }
        }
...
class WorkflowDesigner {
    ...
    getCollision() {
        var collision = this.flow.Activities.map(a => a.collides()).filter(r => r)[0];
    ...
    mousemove(e) {
        if (e.target.localName.toLowerCase() === 'canvas') {
            if (!this.dragging && this.active && this.active.move) {
                ...
                var x = mouseposition.x - this.dragstart.x;
                var y = mouseposition.y - this.dragstart.y;
    ...
    initMenu(container) {
        var menu = $(".flowmenu", container);
        $('[name=settings]', menu).off('click').on('click', this.settings.bind(this));
        $('[name=run]', menu).off('click').on('click', this.flow.run.bind(this.flow));
        $('[name=save]', menu).off('click').on('click', this.flow.save.bind(this.flow));
        this.buildMenu(menu);
    }
    ...
    buildMenu(container) {
        var groups = Array.from(new Set(this.stepTypes.map((t) =>  t.Category )));
    ...
    buildMenuCategory(type) {
        ...
        if (type.Types.length > 0) {
            var categoryName = type.Name.split("`")[0];
            categoryName = categoryName === 'Activity' ? 'Activity' : categoryName.replace('Activity', '');

            var category = $('<ul class="categorytypes" data-category="' + type.Name + '"></ul>');
            category.append('<li class="header"><span class="k-icon ' + this.categoryIconMap(categoryName) + '"></span>' + categoryName + '</li>');
            var sublist = $('<ul class="subtypes"></ul>');
            category.append(sublist);

            type.Types.map(t => {
                var typeName = t.DisplayName.split("`")[0];
                typeName = typeName === 'Activity' ? 'Activity' : typeName.replace('Activity', '');
                var newType = $('<li><span name="add" class="k-icon k-i-add"></span>' + typeName + '</li>');
                newType.data('model', t);
                newType.css('cursor', 'pointer');
                var helper = $('<div class="flow-helper"></div>');
    ...
    menuItemClicked(e, position) {
        var activity = $(e.target).data('model');
    ...
    settings() {
        ...
        loadComponent(container, "FlowSettings", function(c) {
            FlowSettings.init(session.app, container, that.flow, function() { d.close(); });
        });
    ...
    autoArrange() {
        ...
        function placeActivity(prev, a, i) {
            var x = (a.x == 0) ? prev.x + a.w + 100 : a.x;
            var y = prev.y + (i * 1.5);

            a.setPosition(x, y);
            getChildActivities(a).map(function (b, j) { placeActivity(a, b, j * a.h); });
        }

        function getChildActivities(a) {
            var links = that.flow.Links.filter(l => l.model.Source === a.model.Ref);
        ...
        for (var i in that.flow.Activities) {
            var a = that.flow.Activities[i];
            var clash = getActivitiesAt(a.x, a.y);
            if (clash.length > 1) {
