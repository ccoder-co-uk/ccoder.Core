...
namespace Core.Services
{
    public class PageService : CoreService<Page>, IPageService
    {
        ...
        public PageService(ICoreDataContext db, Config config) : base(db) => this.config = config;
        ...
        public RenderResult Render(int appId, string path, string theme, string culture, bool edit = false)
        {
            ...
            {
                // denied ...
                var names = page.Contents.Select(c => c.Name).Distinct();

                page.Contents = new Content[]
                {
                    new Content() { CultureId = "", Html = "[component[login]]", Name = names.First() }
                };
        ...
        async Task SetPageInfoOnNewPage(Page page, Page result)
        {
        ...
        async Task SetContentsOnNewPage(Page page, Page result)
        {
            if (page.Contents != null && page.Contents.Any())
            {
                // add contents
        ...
        void SetRolesOnNewPage(Page page, Page parent, Page result)
        {
            if (page.Roles == null || !page.Roles.Any())
            {
        ...
        async Task CreateChildren(Page page, Page result)
        {
        ...
        bool UserCan(string privKey, int pageId)
            => GetAll()
                .Include(p => p.Roles)
                    .ThenInclude(pr => pr.Role)
