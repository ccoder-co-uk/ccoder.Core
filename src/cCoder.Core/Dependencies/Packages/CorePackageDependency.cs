    ...
    : IPackageTransferManager
{
    ...
    public async ValueTask ImportAppConfigurationAsync(
        ...
        string data)
    {
        JsonObject imported = DeserializeAppConfiguration(data: data);
        ...
        app.DefaultCultureId = GetString(
            node: imported,
            propertyName: nameof(app.DefaultCultureId))
            ?? string.Empty;
        app.Name = GetString(
            node: imported,
            propertyName: nameof(app.Name))
            ?? app.Name;
        app.DefaultTheme = GetString(
            node: imported,
            propertyName: nameof(app.DefaultTheme))
            ?? app.DefaultTheme;
        app.ConfigJson = GetString(
            node: imported,
            propertyName: nameof(app.ConfigJson))
            ?? app.ConfigJson;
        ...
        return new Package
        {
            ...
            [
                new PackageItem
                {
                    ...
                    Data = new JsonObject
                    {
                        [nameof(app.Id)] = app.Id,
                        [nameof(app.DefaultCultureId)] = app.DefaultCultureId,
                        [nameof(app.TenantId)] = app.TenantId,
                        [nameof(app.Name)] = app.Name,
                        [nameof(app.Domain)] = app.Domain,
                        [nameof(app.DefaultTheme)] = app.DefaultTheme,
                        [nameof(app.ConfigJson)] = app.ConfigJson,
                    }.ToJsonString(),
                },
            ],
        };
        ...
        string sourceApi)
    {
        ...
        var rows = await core.Set<PageRole>()
            ...
            .Join(
                ...
                resultSelector: (joined, role) => new
                {
        ...
        {
            items.Add(value: new JsonObject
            {
                ["Path"] = item.Path,
                ["Role"] = item.Role,
            });
    ...
    private static JsonObject DeserializeAppConfiguration(string data)
    {
