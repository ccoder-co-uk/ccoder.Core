...
internal class FormatterExcelFileBuilder(string culture, IEnumerable<Resource> resources)
{
    ...
    public Stream BuildFor(object data)
    {
        ...
        {
            AddRels(excelFile: excelFile);
            AddDocProps(excelFile: excelFile);
            AddXl(excelFile: excelFile);

            if (data is IEnumerable enumerable)
            {
                InsertData(excelFile: excelFile, data: enumerable.Cast<object>());
            }
            else
            {
...
oldValue: "'", newValue: "\""
    ...
    {
        _ = excelFile.CreateEntry(entryName: "xl/", compressionLevel: CompressionLevel.Optimal);
        AddXlFiles(excelFile: excelFile);

        _ = excelFile.CreateEntry(entryName: "xl/_rels/", compressionLevel: CompressionLevel.Optimal);
        AddXlRelsFiles(excelFile: excelFile);

        _ = excelFile.CreateEntry(entryName: "xl/theme/", compressionLevel: CompressionLevel.Optimal);
        AddXlThemeFiles(excelFile: excelFile);

        _ = excelFile.CreateEntry(entryName: "xl/worksheets/", compressionLevel: CompressionLevel.Optimal);
    }
...
<worksheet xmlns='http://schemas.openxmlformats.org/spreadsheetml/2006/main' xmlns:r='http://schemas.openxmlformats.org/officeDocument/2006/relationships' xmlns:mc='http://schemas.openxmlformats.org/markup-compatibility/2006' mc:Ignorable='x14ac xr xr2 xr3' xmlns:x14ac='http://schemas.microsoft.com/office/spreadsheetml/2009/9/ac' xmlns:xr='http://schemas.microsoft.com/office/spreadsheetml/2014/revision' xmlns:xr2='http://schemas.microsoft.com/office/spreadsheetml/2015/revision2' xmlns:xr3='http://schemas.microsoft.com/office/spreadsheetml/2016/revision3' xr:uid='{ECBAD190-FA55-4CFD-8AD0-DA1CA5D1EA29}'><dimension ref='A1'/><sheetViews><sheetView tabSelected='1' workbookViewId='0'/></sheetViews><sheetFormatPr defaultRowHeight='15' x14ac:dyDescent='0.25'/><sheetData>DATA</sheetData><pageMargins left='0.7' right='0.7' top='0.75' bottom='0.75' header='0.3' footer='0.3'/></worksheet>"
                ...
                .Replace(
oldValue: "DATA", newValue: $"{BuildSheetHeaderRow(properties: properties)}{BuildSheetDataRows(data: items, properties: properties)}"
                )
        );
    }
...
handler: $"<c r=\"{index.ToExcelColumn()}1\" t=\"inlineStr\"><is><t>{displayName}</t></is></c>"
    ...
    private string BuildSheetDataRows(IEnumerable<object> data, string[] properties)
    {
        ...
        {
            _ = result.Append(
handler: $"<row r=\"{rowNumber}\" x14ac:dyDescent=\"0.25\" spans=\"1:{properties.Length}\">"
            );
...
value: propertyValue switch
{
    ...
    private static string FormatValue(
        ...
        {
            DateTime dateTime =>
                dateTime.ToString(format: dateFormat, provider: CultureInfo.CreateSpecificCulture(name: culture)),
            DateTimeOffset dateTimeOffset =>
...
format: dateFormat, formatProvider: CultureInfo.CreateSpecificCulture(name: culture)
            ...
            string stringValue => stringValue.Replace(oldValue: "&", newValue: "&amp;"),
            Guid guid => $"{guid}",
            null => string.Empty,
            _ => value.ToString(),
        };
